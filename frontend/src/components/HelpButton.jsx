...
const HELP_CONTENT = {
  ...
  data1c: {
    ru: {
      title: 'Данные 1С — IMAP-приёмка и сводная',
      intro: 'Интеграция с 1С через IMAP: 1С шлёт XLSX-файлы на почтовый ящик, сервис imap1cFetcher забирает их по расписанию, парсер раскладывает в таблицы (план/ремонт/выработка), сводная сводит к актуальному состоянию заказ-нарядов. Под капотом — REST /api/oneC/*. Пять вкладок: Сейчас, Импорты, Несопоставленные, Выработка, Настройки.',
      sections: [
        {
          heading: 'Вкладка «Сейчас»',
          items: [
            'Актуальный реестр заказ-нарядов (последняя версия каждого номера, через ROW_NUMBER OVER).',
            'Поиск по номеру, VIN, госномеру. Фильтр по состоянию.',
            'Колонки: №, Госномер, VIN, Состояние, План, Закрыт, Норма, Исполнитель.',
            'Источник: **OneCWorkOrderMerged** + **OneCStageMerged**, склеенные из последних импортов.',
          ],
        },
        {
          heading: 'Вкладка «Импорты»',
          items: [
            '[upload] Кнопка **«Загрузить xlsx»** — ручная загрузка файла. Тип: plan / repair / performed (или авто-детект по заголовкам).',
            '[refresh] Кнопка **«Цикл IMAP»** — форс-запуск опроса почты прямо сейчас (для теста).',
            'Таблица истории импортов: получено, отправитель, тема, файл, тип, обработано/всего строк, статус.',
            '[●green] **success** — импорт успешен. [●red] **error_*** — есть проблема, в столбце видно сообщение.',
            'Дедупликация по contentHash — повторная загрузка одинакового файла не создаст дублей.',
          ],
        },
        {
          heading: 'Вкладка «Несопоставленные»',
          items: [
            'Список **сырых имён постов** из 1С, которые система ещё не сопоставила с реальным постом.',
            'Колонки: сырое имя, сколько раз встречалось, первый/последний раз, текущая резолюция.',
            'Селект **— не наш пост —** + чекбокс **«отслеживать»**: либо привязать к одному из 10 постов, либо явно пометить как «не наш» (тогда такие записи игнорятся в матчинге и нестыковках).',
            'Регулярные имена («ПОСТ 3», «Пост 5») распознаются автоматом, ручная резолюция нужна для нестандартных написаний.',
          ],
        },
        {
          heading: 'Вкладка «Выработка»',
          items: [
            'Сводка по нормочасам исполнителей за период.',
            'Фильтр **С / По** — диапазон дат закрытия.',
            'Колонки: исполнитель, нормочасы, заказов, виды работ.',
            'Источник: REST **/api/oneC/payroll** — агрегация по closed_at и executor.',
          ],
        },
        {
          heading: 'Вкладка «Настройки» (admin)',
          items: [
            '[lock] Доступ только при пермиссии **manage_1c_config**.',
            '**IMAP host / Порт / SSL** — параметры почтового сервера.',
            '**Логин / Пароль** — пароль шифруется AES-256-GCM перед записью в БД, в ответе API возвращается только **passwordSet: true/false**, никогда не сам пароль.',
            '**Фильтр From** — адрес-источник; письма от других адресов игнорятся.',
            '**Маска темы** (опционально) — дополнительный фильтр по subject.',
            '**Интервал, мин** — период опроса IMAP (5–1440).',
            '**Окно матчинга, ч** — за сколько часов до/после **closed_at** искать соответствующую CV-сессию.',
            '**Удалять после, дней** — авто-чистка OneCImport старше N дней (0/пусто = не удалять).',
            '**Помечать как прочитанные** — выставлять \\Seen flag после успешного импорта.',
            '[refresh] Кнопка **«Проверить соединение»** — реальный коннект к IMAP без записи в БД (требует ввести пароль).',
          ],
        },
        {
          heading: 'Типы файлов из 1С',
          items: [
            '**plan** — реестр планирования (заголовки: Документ, Состояние, Госномер, VIN, Запланировано на, …). Идёт в **OneCPlanRow**.',
            '**repair** — заказ-наряды (Состояние, Закрыт, Норма ч., Исполнитель, …). Идёт в **OneCRepairOrderRow**.',
            '**performed** — выполненные работы (детализация по этапам). Идёт в **OneCStageRow**.',
            'Авто-детект работает по сигнатурам колонок. При неправильном детекте — выберите тип вручную в Импортах.',
          ],
        },
        {
          heading: 'Типичные сценарии',
          items: [
            '[ok] **Плановое утреннее обновление** — 1С отправляет XLSX в почту → imap1cFetcher по расписанию забирает → Импорты показывают success → Сейчас обновляется.',
            '[ok] **Принудительный апдейт** — нажать «Цикл IMAP» в Импортах, не ждать таймера.',
            '[ok] **Резолюция нового поста** — после нового XLSX в Несопоставленных появилось имя → выбрать пост или «не наш» → следующий цикл матчит правильно.',
            '[ok] **Подбор пароля** — Настройки → ввести новый пароль → «Проверить соединение» → если OK, нажать «Сохранить».',
          ],
        },
      ],
    },
    en: {
      title: '1C Data — IMAP intake and merged register',
      intro: 'Integration with 1C via IMAP: 1C sends XLSX files to a mailbox, the imap1cFetcher service picks them up on schedule, a parser splits them into tables (plan/repair/performed), and a merger reduces them to the current state of work orders. Backed by REST /api/oneC/*. Five tabs: Current, Imports, Unmapped, Payroll, Settings.',
      sections: [
        {
          heading: 'Current Tab',
          items: [
            'Current work order register (latest version per order number, via ROW_NUMBER OVER).',
            'Search by number, VIN, plate. Filter by state.',
            'Columns: #, Plate, VIN, State, Plan, Closed, Norm, Executor.',
            'Source: **OneCWorkOrderMerged** + **OneCStageMerged**, merged from latest imports.',
          ],
        },
        {
          heading: 'Imports Tab',
          items: [
            '[upload] **Upload xlsx** button — manual file upload. Type: plan / repair / performed (or auto-detect by headers).',
            '[refresh] **IMAP cycle** button — force-run mail polling now (for testing).',
            'Imports history table: received, sender, subject, file, type, processed/total rows, status.',
            '[●green] **success** — successful import. [●red] **error_*** — failure, message shown in the column.',
            'Dedup by contentHash — re-uploading the same file does not create duplicates.',
          ],
        },
        {
          heading: 'Unmapped Tab',
          items: [
            'List of **raw post names** from 1C that the system has not yet matched to a real post.',
            'Columns: raw name, occurrences, first/last seen, current resolution.',
            'Dropdown **— not our post —** + checkbox **"track"**: either bind to one of 10 posts, or explicitly mark as "not ours" (such records are ignored in matching/discrepancies).',
            'Regular names ("POST 3", "Post 5") are auto-recognized; manual resolution is needed only for non-standard spellings.',
          ],
        },
        {
          heading: 'Payroll Tab',
          items: [
            'Norm-hours summary per executor over a period.',
            '**From / To** filter — closed_at date range.',
            'Columns: executor, norm hours, orders, repair kinds.',
            'Source: REST **/api/oneC/payroll** — aggregation by closed_at and executor.',
          ],
        },
        {
          heading: 'Settings Tab (admin)',
          items: [
            '[lock] Requires **manage_1c_config** permission.',
            '**IMAP host / Port / SSL** — mail server parameters.',
            '**Login / Password** — password encrypted with AES-256-GCM before being stored; API only returns **passwordSet: true/false**, never the password itself.',
            '**From filter** — sender address; mail from other addresses is ignored.',
            '**Subject mask** (optional) — extra subject filter.',
            '**Interval, min** — IMAP polling period (5–1440).',
            '**Match window, h** — how many hours around **closed_at** to look for the matching CV session.',
            '**Delete after, days** — auto-cleanup of OneCImport older than N days (0/empty = never).',
            '**Mark as read** — set \\Seen flag after successful import.',
            '[refresh] **Test connection** — real IMAP connect without DB write (requires password input).',
          ],
        },
        {
          heading: '1C File Types',
          items: [
            '**plan** — planning register (headers: Document, State, Plate, VIN, Scheduled for, …). Stored in **OneCPlanRow**.',
            '**repair** — work orders (State, Closed, Norm h., Executor, …). Stored in **OneCRepairOrderRow**.',
            '**performed** — performed works (stage-level breakdown). Stored in **OneCStageRow**.',
            'Auto-detect works by column signatures. If misdetected — pick type manually in Imports.',
          ],
        },
        {
          heading: 'Common Workflows',
          items: [
            '[ok] **Scheduled morning update** — 1C emails XLSX → imap1cFetcher picks it up on schedule → Imports shows success → Current updates.',
            '[ok] **Forced update** — click "IMAP cycle" in Imports, no need to wait for the timer.',
            '[ok] **New post resolution** — after a new XLSX, a name appears in Unmapped → pick a post or "not ours" → next cycle matches correctly.',
            '[ok] **Password rotation** — Settings → enter new password → "Test connection" → on OK, click "Save".',
          ],
        },
      ],
    },
  ...
  discrepancies: {
    ru: {
      ...
      intro: 'Сводная страница расхождений между планом 1С и реальностью с камер. Детектор гоняет 6 правил поверх **OneCWorkOrderMerged** + **VehicleSession** + **PostStay** и пишет каждое расхождение в **Discrepancy** (open/acknowledged/resolved/dismissed). Уведомления — Telegram (critical немедленно + дайджест) и Socket.IO в браузер.',
      sections: [
        {
          heading: '6 типов нестыковок',
          items: [
            '[●red] **no_show_in_cv** — заказ-наряд закрыт в 1С, но CV не видела авто на территории. Возможен фейк-наряд или авария интеграции.',
            '[●yellow] **no_show_in_1c** — авто стояло на посту > N минут с работником, но в 1С нет соответствующего ЗН. Серая работа?',
            '[●yellow] **wrong_post** — 1С говорит «пост 3», CV видит на «посту 7».',
            '[●yellow] **overstated_norm_hours** — нормочасы > фактического времени на посту в **разы** (завышение).',
            '[●blue] **understated_actual_time** — фактическое время на посту > нормы заметно (занижение в 1С).',
            '[●blue] **time_mismatch** — закрытие в 1С и выезд по CV отличаются на > матчинг-окна.',
          ],
        },
        {
          heading: 'KPI и фильтры',
          items: [
            'Верх страницы — 5 KPI-карточек: Всего, Открыто, За 24 часа, Critical, Warning.',
            'Под KPI — 6 карточек по типам: клик переключает фильтр **type=...**.',
            'Фильтры: статус, severity, номер заказа. По умолчанию открыты только **open**.',
            'Бейдж в сайдбаре (рядом с пунктом «Нестыковки») — счётчик **open** + critical-за-24ч, обновляется каждые 30 секунд.',
          ],
        },
        {
          heading: 'Жизненный цикл',
          items: [
            '[●yellow] **open** — детектор только что нашёл расхождение.',
            '[●blue] **acknowledged** — приняли в работу (acknowledgedBy/At проставляются).',
            '[●green] **resolved** — поправили (в 1С/CV/обоих); resolvedBy/At + closeReason/Comment.',
            '[●gray] **dismissed** — отклонили как ложную (например, ручная сверка показала ОК).',
            'Для возврата в **open** — кнопка «Открыть» в правой колонке (сбрасывает все ack/resolved-поля).',
            'Дубликаты схлопываются по уникальному ключу `(type, orderNumber, postId, vehicleSessionId)` — повторное обнаружение того же не создаёт новую запись.',
          ],
        },
        {
          heading: 'Раскрытая строка (детали)',
          items: [
            'Клик по шеврону слева — раскрывает **two-pane**: 1С слева, CV справа.',
            ...
            'Если у нестыковки записан JSON в `oneCValue` / `cvValue` — он показывается raw для диагностики.',
          ],
        },
        {
          heading: 'Уведомления',
          items: [
            '[zap] **Critical** — Telegram-сообщение немедленно (broadcast в чат).',
            '[zap] **Дайджест** — раз в N часов (по умолчанию 4ч): сводка с топ-3 постами, разбивкой по severity. Управляется env **DISCREPANCY_DIGEST_DISABLED=1**.',
            '[zap] **Socket.IO** — событие `discrepancy:new` сразу шлётся в браузер; счётчик в сайдбаре обновляется без перезагрузки.',
            '[zap] **Тоггл «Нестыковки» в шапке** Dashboard/MapViewer/PostsDetail/MyPost/DashboardPosts включает красные точки на постах с открытыми нестыковками — состояние пишется в `User.uiState.showDiscrepancies`.',
          ],
        },
        {
          heading: 'Permissions',
        ...
        },
        {
          heading: 'Типичные сценарии',
          items: [
            '[ok] **Утренний разбор** — открыть страницу, отфильтровать по **critical+open**, по каждой строке решить: ack → разобраться или dismiss.',
            '[ok] **Серая работа** — нашли **no_show_in_1c** → подняли запись в 1С → resolve с комментарием «оформлено постфактум».',
            '[ok] **Ложный фейк** — критическая **no_show_in_cv** → проверили — клиент действительно не приехал, но мастер забыл отменить ЗН → resolve с reason `wrong_state_in_1c`.',
            '[ok] **После правки 1С** — нажать «Пересчитать» → нестыковка автоматически закрылась (правило больше не срабатывает) или осталась — значит правка не помогла.',
          ],
        },
      ],
    },
