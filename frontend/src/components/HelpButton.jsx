...
const HELP_CONTENT = {
  ...
  data1c: {
    ru: {
      title: 'Данные 1С — приёмка по почте и сводный реестр',
      intro: 'Интеграция с 1С через почту: 1С присылает XLSX-файлы на почтовый ящик, фоновая служба забирает их по расписанию, парсер раскладывает в три таблицы («План», «Ремонты», «Выработка»), сводная сводит данные к актуальному состоянию заказ-нарядов. Пять вкладок: «Сейчас», «Импорты», «Несопоставленные», «Выработка», «Настройки».',
      sections: [
        {
          heading: 'Вкладка «Сейчас»',
          items: [
            'Актуальный реестр заказ-нарядов (последняя версия каждого номера).',
            'Поиск по номеру, VIN, госномеру. Фильтр по состоянию.',
            'Колонки: №, Госномер, VIN, Состояние, План, Закрыт, Норма, Исполнитель.',
            'Источник — сводная таблица заказ-нарядов и этапов, собранная из последних импортов.',
          ],
        },
        {
          heading: 'Вкладка «Импорты»',
          items: [
            '[upload] Кнопка **«Загрузить xlsx»** — ручная загрузка файла. Тип: «План», «Ремонты», «Выработка» (или автоопределение по заголовкам).',
            '[refresh] Кнопка **«Цикл IMAP»** — принудительный запуск опроса почты прямо сейчас (для теста).',
            'Таблица истории импортов: получено, отправитель, тема, файл, тип, обработано/всего строк, статус.',
            '[●green] **Успешно** — импорт успешен. [●red] **Ошибка** — есть проблема, сообщение видно в столбце статуса.',
            'Дедупликация по контрольной сумме — повторная загрузка одинакового файла не создаст дублей.',
          ],
        },
        {
        ...
        {
          heading: 'Вкладка «Настройки» (только для админов)',
          items: [
            '[lock] Доступ только с правом «Управление настройками 1С».',
            '**Адрес IMAP-сервера / Порт / Защищённое соединение** — параметры почтового сервера.',
            '**Логин / Пароль** — пароль шифруется перед записью в БД; интерфейс показывает только «пароль задан / не задан», сам пароль наружу не отдаётся.',
            '**Фильтр по отправителю** — адрес-источник; письма от других адресов игнорируются.',
            '**Маска темы** (опционально) — дополнительный фильтр по теме письма.',
            '**Интервал опроса, мин** — как часто проверяется почта (5–1440).',
            '**Окно матчинга, ч** — за сколько часов до/после закрытия заказа искать соответствующую CV-сессию.',
            '**Удалять после, дней** — авто-чистка истории импортов старше N дней (0/пусто = не удалять).',
            '**Помечать письма как прочитанные** — после успешного импорта.',
            '[refresh] Кнопка **«Проверить соединение»** — реальный коннект к почте без записи в БД (требует ввести пароль).',
          ],
        },
        {
          heading: 'Типы файлов из 1С',
          items: [
            '**План** — реестр планирования (заголовки: Документ, Состояние, Госномер, VIN, Запланировано на, …).',
            '**Ремонты** — заказ-наряды (Состояние, Закрыт, Норма ч., Исполнитель, …).',
            '**Выработка** — выполненные работы с детализацией по этапам.',
            'Авто-определение работает по заголовкам колонок. При неправильном определении — выберите тип вручную в «Импортах».',
          ],
        },
        {
          heading: 'Типичные сценарии',
          items: [
            '[ok] **Плановое утреннее обновление** — 1С отправляет файл в почту → служба по расписанию забирает → в «Импортах» видно «Успешно» → вкладка «Сейчас» обновляется.',
            '[ok] **Принудительный апдейт** — нажать «Цикл IMAP» в «Импортах», не ждать таймера.',
            '[ok] **Резолюция нового поста** — после нового файла в «Несопоставленных» появилось имя → выбрать пост или «не наш» → следующий цикл сопоставит правильно.',
            '[ok] **Смена пароля почты** — «Настройки» → ввести новый пароль → «Проверить соединение» → если ОК, нажать «Сохранить».',
          ],
        },
      ],
    },
          ...
          sections: [
            {
              ...
              items: [
                ...
                '[●purple] **Закр. ЗН ↔ Заказ-наряды и Заявки** — берём закрытые ЗН (последняя версия performed) и сравниваем 4 длительности: Δплан, Δуточн, Δфакт и Δзакр.−старт против нормочасов. Цель — поймать переработки и недоработки.',
          ...
          sections: [
            ...
            },
            {
              ...
              items: [
                ...
                '[●pink] **Δ закр.−старт** *(pink фон)* — closedAt − workStartedAt. Часто длиннее Δфакт: между «закончил работать» и «закрыл наряд» проходит время оформления документов.',
              ],
            },
            {
