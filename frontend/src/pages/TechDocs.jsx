...
\u2502   \u2502                       # reportSchedule, monitoring, settings, backup, replay, cameraMapping
...
# ${isRu ? 'SQLite запускается в WAL-режиме (pragma journal_mode=WAL) для конкурентного чтения' : 'SQLite runs in WAL mode (pragma journal_mode=WAL) for concurrent reads'}`}</Code>
        ...
        <Table
          ...
          rows={[
            ...
            ['cameraMapping', '/api/camera-mapping', isRu ? 'GET / (текущий маппинг камер на зоны/посты), POST / (обновить), DELETE / (сброс к дефолту). Хранится в legacy JSON для обратной совместимости' : 'GET / (current camera-to-zone/post mapping), POST / (update), DELETE / (reset to default). Stored in legacy JSON for backward compatibility'],
          ]}
        />
